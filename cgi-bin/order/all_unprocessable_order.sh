...
all_unprocessable_order () {
    ...
    if [ "$1" -eq "1" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3.3.8/bin/escript /home/gor/rpc_call.escript order_node search_by "[[{'status', 'equal','unprocessable'}], 'key']."
       echo '</pre>'
    elif [ "$1" -eq "2" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3.3.8/bin/escript /home/gor/rpc_call.escript order_node search_by "[[{'status', 'equal', 'unprocessable'}], 'record']."
       echo '</pre>'
    else 
