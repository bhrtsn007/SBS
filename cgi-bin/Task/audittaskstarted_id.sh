...
audit_task_started () {
    ...
    if [ "$2" -eq "1" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3.3.8/bin/escript /home/gor/rpc_call.escript audittaskrec search_by "[[{'pps_id', 'equal',$1},{'status', 'equal', {'pending','started'}}], 'key']."
       echo '</pre>'
    elif [ "$2" -eq "2" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3.3.8/bin/escript /home/gor/rpc_call.escript audittaskrec search_by "[[{'pps_id', 'equal',$1},{'status', 'equal', {'pending','started'}}], 'record']."
       echo '</pre>'
    else 
