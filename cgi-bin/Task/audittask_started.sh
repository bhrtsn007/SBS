...
audit_task_started_system () {
    ...
    if [ "$1" -eq "1" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3.3.8/bin/escript /home/gor/rpc_call.escript audittaskrec search_by "[[{'status', 'equal', {'pending','started'}}], 'key']."
       echo '</pre>'
    elif [ "$1" -eq "2" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3.3.8/bin/escript /home/gor/rpc_call.escript audittaskrec search_by "[[{'status', 'equal', {'pending','started'}}], 'record']."
       echo '</pre>'
    else 
